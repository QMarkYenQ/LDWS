...
JNIEXPORT jint JNICALL Java_com_example_mark_openvideo_myNDK_imgProcess
    ...
    myrgba->copyTo(frame);
        LOGD( "img res %d, %d",frame.rows,frame.cols);
    ...
    if( !frame.empty() ){
		...
		cv::Canny(image,contours,180,350);//WT
		cv::Mat contoursInv;
	...
	//	std::cout << "Lines: (" << li_size << ")\n";
		...
		if(col1 == 1){
			cv::circle(image,cv::Point(230,444), 20, cv::Scalar(0,0,255,255),2,8,0);
		}
		else{
			cv::circle(image,cv::Point(230,444), 20, cv::Scalar(255,0,0,255),2,8,0);
		}


		if(col2 == 1){
			cv::circle(image,cv::Point(380,444), 20, cv::Scalar(0,0,255,255),2,8,0);
		}
		else{
			cv::circle(image,cv::Point(380,444), 20, cv::Scalar(255,0,0,255),2,8,0);
		}
